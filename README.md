...
Таким образом при генерации ленты у пользователя будет смотреться кэш текущих id постов и затем идти в базу для отдачи этих постов.
...
- Posts
    ...
    Считаем место шардов: 2 500 Тб. / 56 = `44 Тб`. Возьмем 64 Тб. HDD.
...
Запросов будет поступать 11 000 в секунду. Redis хранит все в оперативной памяти. Возьмем 256 Гб. RAM, SSD диск на 256 Гб, 32 CPU. Предположим, что один такой сервер сможет справится с нагрузкой в 1 000 rps. Всего понадобится 11 серверов. Также выделим по 2 реплики на каждый сервер. Итого, 11 * 3 = 33 сервера.
